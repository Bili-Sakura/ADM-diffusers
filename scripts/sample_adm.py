...
def main():
    ...
    while produced < args.num_samples:
        ...
        if args.class_label is not None:
            class_labels = torch.full((current_bs,), args.class_label, dtype=torch.long, device=args.device)
        sample = pipeline(
        ...
        if image_dir:
            for i in range(sample.shape[0]):
                Image.fromarray(sample[i]).save(image_dir / f"sample_{produced + i:06d}.png")
        produced += current_bs
